...
<?php    } ?>
    ...
    <?php
      if(isset($_POST['addlang']))
      {
          $name =$_POST['sname'];
          $level=$_POST['level'];
          $sql="UPDATE `language` SET `name` = '$name', `level` = '$level' WHERE `language`.`id` = $edit_id ";
          if(mysqli_query($conn, $sql)){
            echo"<script>window.location='index.php'; </script>";
        }
      }
            $sql= "SELECT * FROM `language` WHERE id=$edit_id";
            $result = mysqli_query($conn, $sql);
        ...
        <h2 class="w3-text-grey w3-padding-16">
            ...
            <form class="w3-container w3-card-4" action="" method='post'>
                ...
                <p>      
                  ...
                  <input class="w3-input w3-border" name="sname" value="<?php echo $row['name'];?>" type="text">
                </p>
                <p>      
                  <label class="w3-text-blue"><b>Skill level</b></label>
                  <input class="w3-input w3-border" name="level"  value="<?php echo $row['level'];?>" type="number">
                </p>
                <p>      
                  <button class="w3-btn w3-blue" name="addlang">Update</button>
                </p>
            </form>
       </div>
